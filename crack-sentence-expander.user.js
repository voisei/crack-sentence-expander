...
(function () {
    ...
    function buildPrompt(dialogue, action, lengthKey, persona, pov, name, style, context, crackMemory) {
        ...
        if (context && context.length) {
            ...
            lines.push('여기에는 유저가 전에 보낸 말뿐 아니라, 크랙/상대 캐릭터가 방금 출력한 대사와 행동도 포함된다.');
    
            context.forEach((m, i) => {
                if (i === context.length - 1) {
                    lines.push('  · [가장 최신/직전 크랙 출력 가능성 높음] ' + m);
                } else {
                    lines.push('  · [이전 흐름] ' + m);
                }
            });

            lines.push('- 이번 출력은 유저가 전에 보낸 문장을 반복하거나 이어 쓰는 것이 아니다.');
            lines.push('- 가장 아래의 최신 맥락, 특히 크랙/상대 캐릭터가 방금 출력한 말·행동·상황에 반응해서 티키타카가 이어지게 써라.');
            ...
            lines.push('- 이미 출력된 직전 크랙 내용에 반응하는 것까지만 허용한다.');
        ...
        if (isThird) {
            ...
            lines.push('- 장면을 풍부하게 하기 위해 새로운 단역·배경 NPC의 대사·행동·묘사는 허용한다.');
        ...
        lines.push('- [행동] 칸은 유저 캐릭터가 무엇을 하는지에 대한 지시/의도다.');
        lines.push('- 그 행동을 실제로 수행하는 장면으로 *이탤릭* 지문으로 묘사하라.');
        lines.push('- 절대로 그 지시문을 명령형 대사로 바꾸지 마라.');
        lines.push('- 누군가가 그것을 소리내어 말하게 하지도 마라.');
        lines.push('- 행동 내용이 추상적이면, 그 행동의 구체적인 내용을 창의적으로 채워 실제로 그것을 해내는 모습으로 보여줘라.');
        ...
        function run() {
            ...
            callGemini(
                ...
                (result, truncated) => {
                    ...
                    flash(
                        truncated ? '한도에 걸려 끝이 잘렸어요. "짧게"로 바꾸거나 다시 뽑아보세요 ⚠️' : '',
                        !!truncated
                    );
