...
(function () {
    ...
    function buildPersonaSuggestPrompt(keywords, storyDetail, recentContext) {
        const system = [
            ...
            '',
            '[출력 형식 — 마크다운]',
            '설명, 머리말, 코드블록(```) 없이 페르소나 본문만 마크다운 문법으로 출력한다.',
            '맨 위에 캐릭터 이름 또는 한 줄 요약을 "# 제목" 형태의 대제목으로 쓴다.',
            '그 아래 각 항목을 "## 소제목"으로 나누고, 내용은 소제목 밑에 문장이나 "- 불릿"으로 자연스럽게 적는다.',
            '아래 소제목들을 사용하되, 해당 정보가 없으면 그 소제목은 생략한다.',
            '## 기본 정보 : 나이대, 성별/젠더 표현(사용자가 준 경우), 직업/신분',
            '## 성격 : 핵심 성향 3~5개 (불릿 권장)',
            '## 말투 : 대사 톤과 대화 습관',
            '## 배경 : 제작자 설정에 맞는 최소한의 과거/직업 배경',
            '## 관계 · 목표 : 현재 시작 상황에서 왜 이 장면에 있는지, 무엇을 원하거나 경계하는지',
            '## 주의 : 상대 캐릭터의 행동이나 감정을 대신 정하지 않는다는 방향',
            '',
            '너무 길거나 장황하지 않게, 각 항목은 핵심만 간결하게 쓴다.',
            '마크다운 표(|)나 이미지 문법은 쓰지 말고, 제목(#, ##)·불릿(-)·굵게(**) 정도만 사용한다.'
        ].join('\n');
