...
// @version      6.12.14
// @author       me
// @description  v6.12.13 전체 기능 + 첫 시작채팅 캐시 보강 + 채팅방별 캐시 지우기 버튼 복구
...
(function () {
     ...
     * 4) 설정의 "이 채팅방 캐시 지우기" 버튼 복구
     * 5) document.body 전체를 상시 감시하지 않고 채팅 영역만 감시
     */
    ...
    function installClearButton() {
        ...
        if (title && /v6\.12\.13/.test(title.textContent || '')) {
            title.textContent = (title.textContent || '')
                .replace(/v6\.12\.13/g, 'v6.12.14');
        }

        let button = panel.querySelector('#se-ctx-clear');
        if (button) return true;

        const previewButton = panel.querySelector('#se-ctx-test');
